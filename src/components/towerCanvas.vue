...
import Worker from "../pages/gameWorker/workers/index.ts?worker";
...
function initWorker() {
  const canvasBitmap = document.querySelector(
    `.${idRef.value} canvas`
  ...
  worker.postMessage(
    {
      ...
      towerList: props.tname
        ? [{ x: 4, y: 3, towerName: props.tname }]
        : props.towerList,
      source: {
